...
export type TokenFrequencyMap = Map<string, number>;
...
function tokenize(textContent: string): string[] {
	return textContent.split(REG_IS_ALPHANUMERIC)
		.map(token => stemmer(token))
		.filter(token => token.length > 0);
}

export function getTokenFrequency(tokens: string[]): TokenFrequencyMap {
	const result = new Map<string, number>();
	for (const token of tokens) {
		const frequency = result.get(token)
		if (frequency)
			result.set(token, frequency + 1);
		else
			result.set(token, 1);
	}
	return result;
